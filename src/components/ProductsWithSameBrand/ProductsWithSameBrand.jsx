...
export default function ProductsWithSameBrand() {
  ...
  let productsWithTheSameBrand = allProducts?.filter(
    (product) => product.brand.name === brandName
  ) || [];


  return <>
    <Helmet><title>products With The Same Brand</title></Helmet>
    {productsWithTheSameBrand.length > 0 ?
      <div className=" w-[85%]  mx-auto py-5">
        <div className=" pt-3  ">
          <div className='grid lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 gap-4 shadow-xl'>
            {productsWithTheSameBrand?.map(function (product, idx) {
