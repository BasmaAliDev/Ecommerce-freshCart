...
export default function Wishlist() {
  ...
  const hasWishlistItems = allWishlist && allWishlist.length > 0;
              ...
              <table className='w-3/4 shadow-lg '>
                <tbody>
                  {allWishlist.map(function (item, idx) {
                    return (
                      <tr className='flex items-center justify-between shadow-lg p-5' key={idx}>
                        <div className='flex items-center w-1/4'>
                          <i
                            onClick={() => deleteProductFromWishlist(item.id)}
                            style={{ cursor: 'pointer' }}
                            class="me-4 cursor-pointer fa-solid fa-x"
                          ></i>
                        ...
                        <div>
                          <button onClick={() => addProductDetailsToCart(item.id)} className='my-5 w-full btn p-2 text-center flex items-center justify-center'>
                            Add to Cart
